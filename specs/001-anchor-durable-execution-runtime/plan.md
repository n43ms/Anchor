...
React 19 · Vite · react-router-dom · Tailwind v4 (React + Vite SPA exclusively; Next.js is prohibited). **Nothing else without maintainer approval** (D-04).
...
| **The architectural rationale** | The four core decisions (Postgres over a broker; Redis excluded from ownership; step-level checkpointing; database-clock expiry), the zombie fencing walkthrough, the single-writer ceiling and its sharding answer, and the preempted weaknesses | Continuous; gated by definition-of-done item 6 |
...
| Single PostgreSQL writer ceiling | Throughput curve diverging from the ideal-linear reference | **Not mitigated — measured and published.** The chart raises the obvious question, and the answer is partition the log by `run_id` or move to a per-shard writer |
