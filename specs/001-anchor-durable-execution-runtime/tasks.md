...
- [x] T595 [US8] Add the generated chaos figures to `README.md`, refreshed automatically by the T528 job (`ops/deploy/refresh_readme_figures.py`) and **never hand-typed** (SC-017)
...
- [x] T616 [US8] Resolve `TODO(LICENSE)` — created `LICENSE` (MIT License) at repository root and linked footer in `README.md`.
- [x] T617 [P] Run the full gate: `ruff check`, `ruff format --check`, `mypy --strict`, `pytest -q` across all suites — verified 100% clean (0 lint/formatting errors, 119 mypy source files typed, 213 tests passed).
