...
export const MiniOperatorConsole: React.FC = () => {
  ...
  React.useEffect(() => {
    const updateScale = () => {
      if (window.innerWidth < 1024) {
        const availableWidth = Math.min(window.innerWidth - 32, 992);
        setScaleFactor(availableWidth / 1024);
      } else {
        setScaleFactor(1);
      }
    };
  ...
  return (
    <DemoProvider>
      <div 
        className="w-full max-w-5xl mx-auto my-6 flex justify-center overflow-hidden"
        style={{ height: scaleFactor < 1 ? `${640 * scaleFactor}px` : "640px" }}
      >
        <div
          style={{
            width: "1024px",
