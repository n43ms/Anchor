...
def http_post(
    path: str, data: dict[str, Any], expect_error: bool = False
...
async def run_real_agent_interruption() -> None:
    print("=" * 65)
    ...
    for _attempt in range(600):
        try:
            ...
            for item in events.get("items", []):
                if (
                    item.get("type") == "TOOL_INTENT"
                    and item.get("payload", {}).get("tool_name") == "draft_email"
                ):
                    intent_found = True
                    print(
                        f"    [!] DETECTED TOOL_INTENT for 'draft_email' (Seq: #{item.get('seq')})"
                    )
                    break
    ...
    for _ in range(30):
        try:
            ...
            st = r.get("status")
            print(f"    - Run #{run_id} Status: {st.upper() if isinstance(st, str) else st}")
            if st == "needs_review":
                print("\n" + "=" * 65)
                ...
                status_code, err_body = http_post(
                    f"/api/runs/{run_id}/resolve", {"resolution": "retry"}, expect_error=True
                )
                print(f"    - HTTP Response Code: {status_code}")
                print(f"    - Rejected Error Response: {json.dumps(err_body, indent=2)}")

                print("\n" + "=" * 65)
                print(f"👉 OPEN YOUR BROWSER NOW:  http://localhost:3000/needs-review/{run_id}")
